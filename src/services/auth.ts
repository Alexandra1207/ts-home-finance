...
export class Auth {
    ...
    public static async processUnauthorizedResponse(): Promise<boolean> {
        ...
        const rememberMe: boolean = userInfoString ? JSON.parse(userInfoString).rememberMe : false;
        // const rememberMe: boolean = JSON.parse(localStorage.getItem("userInfo")).rememberMe;
        ...
        if (refreshToken) {
            ...
            if (response && response.status === 200) {
                ...
                if (result && result.tokens.accessToken && result.tokens.refreshToken) {
